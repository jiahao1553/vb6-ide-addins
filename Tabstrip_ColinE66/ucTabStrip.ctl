...
Private mAutoScrollDirection As Long
...
Dim DragDistance As Long, TabIdx As Long, OverCloseButton As Boolean, DoRefresh As Boolean, BeginDrag As Boolean
   ...
   ElseIf Button = vbLeftButton Then
      'this shouldn't be needed, but is (i.e. when clicking from other programs we don't seem to get a mouse down? Maybe they had captured the mouse?)
      If mMouseDownX = -1 Then mMouseDownX = X
      ...
      If Not mReOrderDragging And Not mScrollDragging Then 'begin drag?
         ...
         If BeginDrag Then 'OK, we're dragging. But which type of drag operation?
            ...
            Else
               ...
               Debug.Print "Dragging " & Tabs(mDraggedTabIndex).Caption & " from position " & mDragFromPosition & " - " & mDragDistance
            End If
            mMouseOverTabIndex = 0
         End If
